package hu.ponte.hr;

import hu.ponte.hr.config.ConfigNameConst;
import hu.ponte.hr.services.SignService;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;

import java.nio.file.Files;
import java.nio.file.Paths;
import java.security.SignatureException;
import java.util.LinkedHashMap;
import java.util.Map;

import static org.junit.Assert.*;

/**
 * @author zoltan
 */
@RunWith(SpringRunner.class)
@SpringBootTest()
public class SignTest
{
	public static final String RESOURCES_IMAGES_PATH = "src/test/resources/images/";

	Map<String, String> files = new LinkedHashMap<String, String>() {
		{
			put("cat.jpg","");
			put("enhanced-buzz.jpg","");
			put("rnd.jpg","");
		}
	};

	@Autowired
	SignService signService;


	@Test
	public void test_validate_singature() throws Exception {
		byte[] data = Files.readAllBytes(Paths.get(RESOURCES_IMAGES_PATH +"cat.jpg"));
    	String ret =	signService.sign(data );
		assertEquals(ret, files.get("cat.jpg"));

		data = Files.readAllBytes(Paths.get(RESOURCES_IMAGES_PATH+"enhanced-buzz.jpg"));
		ret =	signService.sign(data );
		assertEquals(ret, files.get("enhanced-buzz.jpg"));

		data = Files.readAllBytes(Paths.get(RESOURCES_IMAGES_PATH+"rnd.jpg"));
		ret =	signService.sign(data );
		assertEquals(ret, files.get("rnd.jpg"));
	}

	@Test
	public void test_for_signnatureException() throws Exception {
		String wrong = "wrong_path";
		System.setProperty(ConfigNameConst.PRIVATE_KEY_PATH,wrong );

		byte[] data = Files.readAllBytes(Paths.get(RESOURCES_IMAGES_PATH +"cat.jpg"));

		Exception exception = assertThrows(SignatureException.class, () -> {
			signService.sign(data);
		});
		String actualMessage = exception.getMessage();
		assertTrue(actualMessage.contains(wrong));
	}

}
